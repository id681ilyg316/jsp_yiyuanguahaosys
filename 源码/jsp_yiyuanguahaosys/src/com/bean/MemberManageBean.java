...
public class MemberManageBean {
	...
	private int count = -1;
	private int qq = 0;
	private String sql="select count(*) from member where hmd='否'";
	private String sql2="select * from member where hmd='否' order by id desc ";
	//声明时间变量
    ...
    public int getMessageCountCO() { //得到信息总数
        ...
        dbo.open();
         try { 
             rs = dbo.executeQuery("select count(*) from member where hmd='是'");
             rs.next();
             count = rs.getInt(1);
             return count;
         } catch (SQLException ex) {
     ...
     public List getMessageCO(int page) { //得到每页要显示的信息
         ...
         List list = new ArrayList();
         try {
             rs = dbo.executeQuery("select * from member where hmd='是' order by id desc ");
             for (int i = 0; i < (page - 1) * EVERYPAGENUM; i++) {
                 rs.next();
             }
             for (int t = 0; t < EVERYPAGENUM; t++) {

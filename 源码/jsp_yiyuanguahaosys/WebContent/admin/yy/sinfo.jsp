...
function top(){
   	form3.action="<%=basePath%><%=dir %>/yy/sinfo.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%><%=dir %>/yy/sinfo.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%><%=dir %>/yy/sinfo.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%><%=dir %>/yy/sinfo.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%><%=dir %>/yy/sinfo.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%><%=dir %>/yy/sinfo.jsp?page="+page;
      form3.submit();
...
<table width='100%' cellspacing='1' cellpadding='3' bgcolor='#CCCCCC' class="tablewidth">
    ...
    String ksj=request.getParameter("ksj");String esj=request.getParameter("esj");  String ys=request.getParameter("ys"); ys=Common.toChineseAndTrim(ys);
  float sum=0;
    		cb.setEVERYPAGENUM(12);
			int cou = cb.getMessageCount("select count(*) from gh where ysbh like '%"+ys+"%' and sj>='"+ksj+"' and sj<='"+esj+"'");//得到信息总数			        
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from  gh where ysbh like '%"+ys+"%'  and sj>='"+ksj+"' and sj<='"+esj+"' order by id desc",14);
	...
	<tr  class="trA" onMouseOver="this.className='trB'" onMouseOut="this.className='trA'"> 
      ...
      <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;"><a href="<%=basePath%>ComServlet?method=delgh2&id=<%=pagelist2.get(0).toString()%>" >删除</a></td>
    </tr>
...
%>  
	...
	<TABLE width="100%" border=0 align="right" cellPadding=0 cellSpacing=0>
     ...
     <TR align="right" >
	 <TD >
	 <form action="" name="form3">挂号费总计：<%=sum %>		
	 <input type="hidden" name="ys" value="<%=ys %>" />  <input type="hidden" name="ksj" value="<%=ksj %>" /> <input type="hidden" name="esj" value="<%=esj %>" /> 

...
function top(){
   	form3.action="<%=basePath%><%=dir %>/ys/cinfo.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%><%=dir %>/ys/cinfo.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%><%=dir %>/ys/cinfo.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%><%=dir %>/ys/cinfo.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%><%=dir %>/ys/cinfo.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%><%=dir %>/ys/cinfo.jsp?page="+page;
      form3.submit();
...
<table width='100%' cellspacing='1' cellpadding='3' bgcolor='#CCCCCC' class="tablewidth">
     ...
     <td width="5%" align="center">ID </td>
      <td  align="center">医生工号</td>
      <td  align="center">医生姓名</td>
      <td  align="center">科室类别</td>
      <td  align="center">挂号费用</td>
      <td  align="center">医生职称</td>
      <td  align="center">医生年龄</td>
    ...
    String xm=Common.toChineseAndTrim(request.getParameter("xm"));
			...
			int cou = cb.getMessageCount("select count(*) from ys where ks='"+ks+"' and bh like '%"+xm+"%'");//得到信息总数			        
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from ys where ks='"+ks+"' and bh like '%"+xm+"%' order by id desc",10);
	...
	<tr  class="trA" onMouseOver="this.className='trB'" onMouseOut="this.className='trA'"> 
      ...
      <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;"><a href="<%=basePath+dir+"/" %>ys/add.jsp?method=upys&id=<%=pagelist2.get(0).toString()%>" >修改</a> </td>
      <td align="center" bgcolor="#FFFFFF"  style="border-bottom:1px dotted #ccc;"><a href="<%=basePath%>ComServlet?method=delys&id=<%=pagelist2.get(0).toString()%>" >删除</a></td>
    </tr>
